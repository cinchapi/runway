...
#### Version 2.3.0 (TBD)
* **Fixed hierarchy queries that used a key declared only by a descendant.** `findAny` and `countAny` now push those criteria to the database instead of loading and filtering the full hierarchy locally. Exact-class queries and criteria that name an unknown key keep their existing behavior. ([GH-186](https://github.com/cinchapi/runway/issues/186))
