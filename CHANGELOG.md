...
* **A `Record` can declare the values a save must verify.** Call `watch` with the names of the fields a decision rests on, and every later save of that `Record` fails with a `StaleDataException` when the database no longer holds the value one of them had when the `Record` last loaded or saved. The declaration covers fields of that `Record`, applies whether or not the save prevents stale writes, and is refused when a name does not identify a stored field. A decision that rests on another `Record` still calls for a `Transaction`, whose reads join its conflict footprint.
