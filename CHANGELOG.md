...
#### Version 2.3.0 (TBD)
* **An anonymous `Audience` can operate transactionally.** An anonymous audience holds the database it operates against, so `startTransaction`, `transact`, `transactAndSupply`, `scope`, `create`, `intern` and the atomic find-and-update operations work for every `Audience`, with the same visibility and permission mediation as a `Record`-based audience; previously the anonymous audience refused transactional operations. `Audience.anonymous()` resolves against the single open `Runway` (also available as `Runway#anonymous()`), and the new `Audience.anonymous(DatabaseInterface)` names the database explicitly, including a `Transaction`.
    ...
    * An anonymous `Audience` that resolves against zero or multiple open `Runway` instances names no database. It answers access policy questions, such as `$checkIfVisible`, and refuses every database operation with an `IllegalStateException`.
    ...
    * A `DatabaseInterface` can `create` a `Record` bound to it, so a direct save persists within that context. A created `Record`, and every `Record` reachable from its constructor arguments, is bound to the creating context. Creation is an optional operation that an implementation which cannot support it refuses with an `UnsupportedOperationException`.
    * A mediated `create` or `intern` leaves the caller's records bound as they were unless it saves the record. A `create` that throws restores the binding of every record reachable from the constructor arguments. An `intern` that returns an existing match, or throws before its save begins, restores the record and every record reachable from it. After a save failure poisons a `Transaction`, its failed-save contract governs.
...
* **`intern` atomically returns the one record that shares a record's unique identity, or saves the given record when none exists.** A record's identity is its data under its `@Unique` constraints, so a caller can construct a record and canonicalize it in one call, with no hand-built criteria that can drift from the constraints. A separate find-then-create sequence can lose a race with a concurrent caller; the single call cannot.
    ...
    * `Record#intern()` is an identity operation, not an audience-mediated action: it canonicalizes the record itself, so no access checks apply, even when the record is an `Audience`. `Audience#intern` is the audience-mediated form, which enforces the audience's permissions and visibility on whatever record it interns, including the audience itself.
...
* Fixed a bug where a save that enforced a `@Unique` constraint conflicted with concurrent writes to unrelated records of the same class, including the creation of a record that holds none of the constrained values. The conflict failed the commit of a `Transaction` that performed the save and made a managed save retry. Such a save now conflicts only with a writer of the constrained values. ([GH-176](https://github.com/cinchapi/runway/issues/176))
