...
* **An `Audience` performs `intern` and the atomic `find*AndUpdate` methods under its access rules.**
    ...
    * A `find*AndUpdate` call updates only a record that is visible to the `Audience`, and only a key the `Audience` may write. A match that is not visible behaves as no match and returns `null`; a key that is not writable is rejected with `RestrictedAccessException`. The access checks evaluate the matched record's state when the operation begins.
