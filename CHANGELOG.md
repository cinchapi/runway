...
##### Bug Fixes
* Fixed a bug where a query for a single record through an `Audience` whose visibility `Scope` denies all access failed with a `ClassCastException` instead of returning `null`. `findUnique`, `findAnyUnique` and `findFirst` now return `null` under such a `Scope`, consistent with their contract when no record matches.
