...
* Fixed a latent bug where `Audience#frame` (and the RESTQL `select` path that delegates to it) returned a positively named key whose value is a terminal `Record` &mdash; or a collection of `Record` values &mdash; as the raw `Record` instead of framing it into a nested map. A key reached by naming it without a navigation suffix (bare or `+`-prefixed) was treated as a terminal leaf and emitted verbatim, whereas the same key, when included implicitly as a default, was recursively framed. Naming a `Record`-valued key therefore yielded a different, un-framed result than letting it ride in on the defaults, and the raw `Record` surfaced downstream as an opaque reference rather than nested data. The fix frames a named terminal `Record` (or collection of `Record` values) with the target's own defaults, identical to the default-included path; scalar leaves are unchanged. The bug predates the lazy-`@Computed` default but was newly exposed by it, since a `@Computed` `Record`-valued key (e.g., a collection of access or exchange records) must now be positively named to appear. ([#135](https://github.com/cinchapi/runway/pull/135))
