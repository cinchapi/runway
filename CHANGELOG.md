...
* **An `Audience` starts and scopes transactions.** `Audience#stage` (or its alias, `startTransaction`) returns an open `Transaction` that the audience joins, so the operations it performs, and the access checks that gate them, resolve within the transaction.
    ...
    * `Audience#run` and `Audience#supply` execute work in the audience's transactional scope, the same as `Record#run` and `Record#supply`.
