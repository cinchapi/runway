...
* **`intern` and the atomic `find*AndUpdate` methods are optional operations on `DatabaseInterface`.**
    * The default implementations throw `UnsupportedOperationException`. `Runway` and every `TransactionInterface` support all of them; on a `TransactionInterface`, the lookup and the update stage within the transaction and commit with it. After a `Transaction` ends, the operations run atomically against the enclosing `Runway`.
* **An `Audience` performs `intern` and the atomic `find*AndUpdate` methods under its access rules.**
    * `intern` requires that the `Audience` is permitted to create the record, even when an existing record already claims the identity, and it refuses an existing match that is not visible to the `Audience`. The refusal of a hidden match still confirms that a record with the identity exists.
    * The `Audience` is recorded as the author of a record that `intern` saves.
    * A `find*AndUpdate` call matches among the records that are visible to the `Audience`, so "first" and "unique" are evaluated over the visible matches. The call updates the match only if the key is writable by the `Audience`; otherwise, it returns `null` and changes nothing. The same field eligibility and replacement rules apply as on `Runway`. The lookup, the access checks and the update run in the `Audience`'s transactional scope: within an open transaction they stage and commit with it; otherwise, they commit together in their own transaction and conflicts retry within the bounds of the governing `AtomicRetryPolicy`.
...
##### Bug Fixes
* Fixed a bug where a `write` performed through an `Audience` could modify a record that a visibility `Scope` hid from that `Audience`, as long as the caller held the record and its field rules permitted the keys. A write through an `Audience` now also requires the record to be visible.
