...
* **Added single-key atomic operations to `Record`.** A caller that holds a record can now conditionally replace one field's value in a single atomic step, without a raw Concourse connection.
    * `Record#exchange(String, Object)` is the conditional counterpart to `Record#set(String, Object)`: `set` stages a value that takes effect on the next save, whereas `exchange` writes through to the database immediately, and only if the database still holds the record's in-memory value for the key. On success, the in-memory field updates to match; on failure, nothing changes anywhere.
    ...
    * A retry touches only the key under update. Staged realm changes and loaded linked records elsewhere in the record survive a retry untouched. If the value of the key itself is a linked record with staged changes, the retry fails instead of replacing it, and if the stored value is concurrently removed, the operation fails because there is nothing left to update.
    * Only an intrinsic single-value field with a non-null value on both sides is eligible, and the replacement must be an instance of the field's type. `@Unique` fields are refused because uniqueness cannot be enforced within a single-key operation. `@Required` and `@ValidatedBy` constraints are enforced on the replacement before anything is written. The `DynamicWritePolicy` that governs `Record#set` also governs these operations, so a field the policy refuses throws `NonWritableFieldException`.
    * A linked-record replacement must have no staged changes, including a record that was never saved, a record with unsaved realm changes and a record staged for deletion. The operation stores a link to the replacement without saving it, so the refusal ensures a stored link never points at state the database does not hold. The replacement must be persisted in the same database as the record whose field is written.
    * These operations are targeted writes, not saves: the `beforeSave` hook and save listeners do not run, and a successful exchange counts as a modification for stale-write detection.
* **Added `AtomicRetryPolicy` to govern how atomic operations respond to contention.** Configure the retry limit and the jittered backoff pacing per `Runway` instance with `Runway.builder().atomicRetryPolicy(AtomicRetryPolicy.create(limit, backoffMillis))`. The configured policy is readable via `runway.properties().atomicRetryPolicy()`. When the retries are exhausted, the operation throws the new `RetryExhaustedException` instead of returning a non-committed result.
