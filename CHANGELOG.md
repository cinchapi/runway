...
* Fixed a bug where `Audience#frame` serialized a `Record`-valued key (or a collection of `Record` values) differently depending on how it was requested. An explicitly named key returned a bare reference to the record; the same key, when included implicitly by default, returned the fully nested object. Both now return the nested object. The inconsistency surfaced once `@Computed` properties became lazy: they must now be named explicitly, so any `@Computed` key with a `Record` value was affected.
