...
* Removed the client-side sorting and pagination fallback for Concourse servers older than `0.10.0`. Any read that supplies an `Order` or `Page` now requires a server with native sorting and pagination support (Concourse `0.10.0` or newer). The fallback, introduced in Runway `1.4.0`, loaded the entire result set, then sorted and paginated it in memory. Callers connected to a pre-`0.10.0` server must upgrade Concourse to continue issuing sorted or paginated reads.
