...
The round-trip impact is sharpest on graphs shaped like a deep tree &mdash; a record type with a self-referential `Collection<Record>` field (`Exchange.children: Set<Exchange>`) whose nodes carry a `parent` back-link and each link out to a handful of other record types, some of them subtypes of a shared polymorphic hierarchy. On the `1.14.x` line (Concourse `0.12.x`), self-referential pre-fetch was bounded at a single level, so loading the root of such a graph `D` levels deep cost on the order of one server round trip per level of depth, growing without bound as the tree deepened. On `2.0.0` (Concourse `1.0.x`), the `*` transitive modifier (`children*`) directs a single `navigate()` to follow the self-referential edge to arbitrary depth, so the entire tree &mdash; every level, together with the other record types reachable from each node &mdash; is pre-fetched in one `navigate()`. The follow-up cleanup `select()` runs only for destinations the `*` modifier cannot express (alternating-field mutual-reference cycles, dangling links), so for a self-referential tree it has nothing left to resolve.
...
Each cell is the upper bound on server round trips to fully resolve one `load` or `find`. `N` = linked records reached; `L` = longest non-cyclic `Record`-to-`Record` chain, a fixed property of the schema; `D` = self-referential tree depth, set by the data and unbounded at runtime. On the `1.14.x` line self-referential pre-fetch was bounded at a single level, so no strategy &mdash; `NONE`, `BULK_SELECT`, or `NAVIGATE` &mdash; resolved a depth-`D` tree in a depth-independent number of round trips. The `2.0.0` row assumes a Concourse `1.0.x` server and a graph the `*` modifier fully reaches; residue it cannot express (alternating-field mutual-reference cycles, dangling links) adds that residue's depth.

This is independent of, and compounds with, the Command API batching described above: on a Concourse `1.0.x` server the `navigate()` pre-fetch is `prepare()`/`submit()`-batched into the same round trip as the originating `load` or `find`, so the whole deep graph resolves end to end in a single round trip. On a pre-`1.0.0` server the transitive `navigate()` still collapses the depth dimension to one call, but without the Command API that call costs one round trip beyond the originating read.
