...
* Fixed a bug where a record's unsaved changes were lost after a failed save when that save also processed an internally loaded copy of the same record for deletion cleanup. The record reported no unsaved changes even though nothing committed, so a later save skipped its fields.
##### Atomic Operations
