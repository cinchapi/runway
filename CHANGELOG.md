...
##### Delta Saves
* **Breaking change: a save now writes only what changed.** Every `Record` remembers the state it last loaded or saved, and a save stages only the difference between the record's current state and that remembered state. Previously, a save wrote the record's entire remembered state, so an instance whose view was stale erased changes that other writers committed after it was loaded. ([GH-163](https://github.com/cinchapi/runway/issues/163))
    * A collection field stages an add for each element the instance introduced and a remove for each element it dropped. Elements that other writers added or removed concurrently survive the save. The same holds when the collection is emptied: only the elements the instance knew about are removed.
    ...
    * Realm membership changes stage per-realm adds and removes, so realm changes made through separate instances merge instead of the later save erasing the earlier one.
    * A mutation with no serialized effect, such as reordering a `List` (the database stores an unordered set of values), is no longer an unsaved change: a save of it writes nothing and fires no save notification.
    * A save is a statement of the changes the instance made, not of complete record state. Re-saving a record no longer repairs stored data that drifted through writes made outside of Runway, and assigning a collection wholesale no longer guarantees that storage exactly matches the assigned collection when other writers changed the stored set concurrently.
    * **Mitigation:** the new `@MergeStrategy` annotation declares how a save merges a field into stored state. Every field without the annotation uses the default `MERGE` strategy described above. Declare `@MergeStrategy(OVERWRITE)` on a field to keep the legacy semantics for that field alone: whenever the record saves, the field writes its full current state, so storage exactly matches the instance and concurrent changes to that field are overwritten. This is the correct semantics for a field whose value set is one logical whole (e.g., an ordered sequence that is always rebuilt in place); never use it on a set that multiple writers extend independently.
    * These semantics apply to every save path, including saves within a `Transaction` and the recursive saves of linked records.
