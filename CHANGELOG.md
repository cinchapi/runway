...
* **Added single-key atomic operations to `Record`.** A caller that holds a record can now conditionally replace one field's value in a single atomic step, without a raw Concourse connection.
    ...
    * A retry touches only the key under update. Staged realm changes and loaded linked records survive a retry untouched, and if the stored value is concurrently removed, the operation fails because there is nothing left to update.
    * Only an intrinsic single-value scalar field with a non-null value on both sides is eligible, and the replacement must be an instance of the field's type. `@Unique` fields are refused because uniqueness cannot be enforced within a single-key operation. Link-typed fields (a `Record` or `DeferredReference` value) are refused because the linked record's own state cannot be covered by a single-key operation. `@Required` and `@ValidatedBy` constraints are enforced on the replacement before anything is written. The `DynamicWritePolicy` that governs `Record#set` also governs these operations, so a field the policy refuses throws `NonWritableFieldException`.
