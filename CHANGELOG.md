...
##### Bug Fixes
* Fixed a bug where a save of a record that another writer deleted restored that record instead of failing. The record reappeared in queries over its class, and a subsequent load of it could fail because the values it requires were gone. A save that would write anything into a record that holds no data now throws the new `DeletedRecordException`, which names the record, and writes nothing; the refusal reaches the caller ahead of a stale-write failure when both apply. A record that has never been saved is unaffected, deleting a record that another writer already deleted remains successful, and a save with nothing to write still succeeds. A realm change is a write, so a save whose only change is realm membership is refused the same way. A save that verifies existence issues one additional server round trip per save operation, not per record, and none when the save has nothing to write or already performs a uniqueness or stale-data read.
