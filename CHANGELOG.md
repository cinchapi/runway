...
* **An `Audience` starts and scopes transactions.** `Audience#startTransaction` returns an open `Transaction` that behaves the same as the audience, just within the confines of the transaction: reads observe the audience's visibility and writes require its permissions.
    ...
    * An `Audience` that is not a `Record` (e.g., the anonymous audience) cannot start a transaction, because it holds no database of its own to start one against. It can still be scoped to a transaction that a caller supplies, and the resulting view applies its visibility to every read and resolves that read within the transaction.
