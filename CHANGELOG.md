...
* **`intern` and the atomic `find*AndUpdate` methods are optional operations on `DatabaseInterface`.**
    * The default implementations throw `UnsupportedOperationException`. `Runway` supports all of them; a `Transaction` supports `intern`.
* **An `Audience` performs `intern` and the atomic `find*AndUpdate` methods under its access rules.**
    * `intern` requires that the `Audience` is permitted to create the record, even when an existing record already claims the identity, and it refuses an existing match that is not visible to the `Audience`.
    * The `Audience` is recorded as the author of a record that `intern` saves.
    * A `find*AndUpdate` call updates a record only if the record is visible to the `Audience` and the key is writable by the `Audience`; otherwise, the call returns `null` and changes nothing. The access checks evaluate the matched record's state when the operation begins.
