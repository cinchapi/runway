...
* **Added `Runway#transaction` to scope reads and writes to a single ACID transaction.** `transaction()` returns a `Transaction`: a `DatabaseInterface` view whose reads observe the transaction's isolated snapshot, including its own uncommitted writes, and join its conflict footprint, so a commit fails instead of persisting a decision that was made on data a concurrent writer changed. ([#165](https://github.com/cinchapi/runway/pull/165))
    * Every `Record` loaded through the view, including the linked records reachable from it, is bound to the transaction: `save()` stages within it and the writes become durable only when `commit()` succeeds. New records join through `Transaction#save`. Save and delete notifications dispatch only after a successful commit. An abort discards every staged write; a record's in-memory edits remain, the same as after a failed save.
    * `Transaction` is `AutoCloseable` (`close()` aborts whatever was not committed) and is confined to the thread that starts it. After it ends, the view forwards reads and saves to the enclosing `Runway`, so bound records unwind to the database scope; only another `commit()` is refused. `runway.run(work)` executes work within a transaction and commits after it completes: the work reads through the provided `DatabaseInterface`, every `record.save()` during the work joins the transaction, new records included, and conflicts retry within the bounds of the governing `AtomicRetryPolicy`.
