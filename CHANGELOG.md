...
* **Added single-key atomic operations to `Record`.** A caller that holds a record can now conditionally replace one field's value in a single atomic step, without a raw Concourse connection.
    ...
    * Only an intrinsic single-value field with a non-null value on both sides is eligible. `@Unique` fields are refused because uniqueness cannot be enforced within a single-key operation. `@Required` and `@ValidatedBy` constraints are enforced on the replacement before anything is written.
