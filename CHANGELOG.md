...
* **Added single-key atomic operations to `Record`.** A caller that holds a record can now conditionally replace one field's value in a single atomic step, without a raw Concourse connection.
    ...
    * `getAndUpdate` and `updateAndGet` refuse a record that has unsaved changes, including a record that was never saved. A retry refreshes the record from the database, and the refusal prevents a silent discard of the staged state.
