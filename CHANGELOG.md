...
##### Bug Fixes
* Fixed a bug where a save of a record that another writer deleted restored that record instead of failing. The saved changes, along with the internal marker that makes a record findable by class, recreated a record that no writer intended to exist, and a subsequent load of it could fail because the values it requires were gone. A save whose record no longer holds data now throws the new `DeletedRecordException`, which names the record, and writes nothing. A record that has never been saved is unaffected, deleting a record that another writer already deleted remains successful, and a save of a record with no changes still writes nothing. A save that verifies existence issues one additional server round trip per save operation, not per record, and none when the save already performs a uniqueness or stale-data read.
