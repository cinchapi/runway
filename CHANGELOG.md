...
* **A `Record` can declare the values a save must verify.** Call `verifyOnSave` with the names of the fields a decision rests on, and the next save of that `Record` fails with a `StaleDataException` when another writer changed one of them after the `Record` loaded or last saved. A decision that rests on another `Record` still calls for a `Transaction`, whose reads join its conflict footprint.
    * A value stored alongside the ones the `Record` saw is a change, so an element another writer added to a declared collection fails the save.
    ...
    * Within a `Transaction`, a `Record` that the `Transaction` loaded needs no declaration, because the `Transaction` fails its own commit when a writer changes anything it read. A save is refused when it carries a declaration on a `Record` that reached its state outside the `Transaction`.
