...
* **Added single-key atomic operations to `Record`.** A caller that holds a record can now conditionally replace one field's value in a single atomic step, without a raw Concourse connection.
    ...
    * `Record#getAndUpdate(String, UnaryOperator)` and `Record#updateAndGet(String, UnaryOperator)` apply a function to the current value and persist the result with the same conditional semantics. When a concurrent writer wins the race, the record refreshes and the function re-applies to the fresh value, so the function may run more than once and must be safe to do so. `getAndUpdate` returns the replaced value; `updateAndGet` returns the produced one.
    ...
    * `getAndUpdate` and `updateAndGet` refuse a record that has unsaved changes, including a record that was never saved and a record with unsaved realm changes. A retry refreshes the record from the database, and the refusal prevents a silent discard of the staged state.
    * Only an intrinsic single-value field with a non-null value on both sides is eligible, and the replacement must be an instance of the field's type. `@Unique` fields are refused because uniqueness cannot be enforced within a single-key operation. `@Required` and `@ValidatedBy` constraints are enforced on the replacement before anything is written.
    * A linked-record replacement must have no unsaved changes, including a record that was never saved. The operation stores a link to the replacement without saving it, so the refusal ensures a stored link never points at state the database does not hold.
