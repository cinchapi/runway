...
* Single-key atomic operations (`getAndUpdate`, `updateAndGet`, `exchange`) require a direct `Runway` binding and are refused on a transaction-bound `Record`, because the transaction's commit is the unit of atomicity.
