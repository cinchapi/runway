...
* Removed the `Gateway` class and the `DatabaseInterface.gateway()` method. Use the `Selection` API (`select(Selection...)`) instead; it provides the same routing for optional `Criteria`, `Order`, and `Page` arguments and batches multiple operations into fewer round trips.
* Removed long-deprecated convenience methods. `Runway.connect(...)` is replaced by `Runway.builder()`; `Runway.findOne(...)` by `Runway.findUnique(...)`; `Runway.onSave(...)` by `Runway.properties().onSave(...)`; and `Runway.Builder.recordsPerSelectBufferSize(int)` by `Runway.Builder.streamingReadBufferSize(int)`.
* Removed long-deprecated `Record` methods. `Record.get(String...)` is replaced by `Record.map(String...)`; the `Record.json(boolean, ...)` overloads by the `Record.json(SerializationOptions, ...)` overloads; the overridable `computed()` and `derived()` hooks by the `@Computed` and `@Derived` annotations; and the overridable `jsonTypeWriters()` and `jsonTypeHierarchyWriters()` hooks by `typeAdapters()`.
* Removed the `Runway.Properties.supportsPreSelectLinkedRecords()` method. Pre-selection of linked `Record` data is always available on the Concourse versions this release supports, so the method always returned `true`; remove any calls to it.
