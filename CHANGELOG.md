...
* **Records join a transaction explicitly.** Every `Record` loaded through the view, including the linked records reachable from it, is bound to the transaction: `save()` stages within it and the writes become durable only when `commit()` succeeds. New records join through `Transaction#save`, or by being created with `Transaction#create`, which binds them to the transaction so a direct `save()` stages within it. A record bound elsewhere saves against its own binding, outside of the transaction, and a `Record` never owns a transaction's lifecycle. A record bound to an open transaction saves only through it; a save through a different scope is refused, whether the record is a direct argument of the save or is reached through the links of another record. A record referenced only through a transient field is outside the saved graph and does not save with its holder. A record that a `beforeSave` hook introduces joins the save under the same boundary check, and within a transaction it is bound to the transaction. `Record#assign` also refuses to move a record out of an open transaction, so an explicit reassignment cannot bypass the boundary.
* **`Transaction` is `AutoCloseable` and thread-confined.** `close()` aborts whatever was not committed, so try-with-resources guarantees a clean end, and the transaction is confined to the thread that starts it. An abort discards every staged write; a record's in-memory edits remain, the same as after a failed save. Save and delete notifications dispatch only after a successful commit. After a transaction ends, the view forwards reads and saves to the enclosing `Runway`, so bound records unwind to the database scope; only another `commit()` is refused.
* **A failed save poisons the transaction.** If a save throws within an open `Transaction`, then the writes that were staged before the failure can never commit: every subsequent operation is refused except `abort()` (or `close()`), so a partial save never becomes durable.
