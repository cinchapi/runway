...
##### Atomic Operations
This release adds single-key atomic operations to `Record`: `exchange`, `getAndUpdate`, and `updateAndGet`. They give a caller that already holds a record a way to conditionally replace one scalar field's value in a single atomic step, without a raw Concourse connection.
