...
* **`findUniqueOrCreate` and `findAnyUniqueOrCreate` atomically return the one record that matches a criteria, creating it when none exists.** The caller supplies a factory that returns a new, unsaved record matching the criteria; the lookup and the save commit as one transaction, so a consumer that hand-rolls find-then-create can rely on a single call with no lost race. `findAnyUniqueOrCreate` applies the same contract across the class hierarchy, as `findAnyUnique` does for `findUnique`.
    * On `Runway`, the operation runs in its own transaction, independent of any transaction the caller holds open: concurrent callers for the same criteria converge on a single record (the loser of a creation race retries and adopts the winner), and conflicts retry within the bounds of the governing `AtomicRetryPolicy`. On a `TransactionInterface`, the lookup and the save stage within that transaction.
    * The factory runs only when no record matches. It may run more than once, so it must be free of side effects, and a factory result that is `null` or does not match the criteria is rejected with `IllegalArgumentException` instead of persisting a record the lookup cannot return. A mismatch within a caller-owned transaction poisons it, so the staged save can never commit.
    * `DuplicateEntryException` propagates when more than one record matches, consistent with `findUnique`.
* **`intern` atomically returns the one record that shares a record's unique identity, or saves the given record when none exists.** A record's identity is its data under its `@Unique` constraints, so a caller can construct a record and canonicalize it in one call, with no hand-built criteria that can drift from the constraints. On `Runway`, the lookup and the save commit as one transaction; on a `TransactionInterface`, they stage within that transaction.
    * A record shares the identity only if it agrees with every `@Unique` constraint; a `null` value does not participate. An existing record that shares some but not all of the identity is not a match; the save then fails `@Unique` enforcement rather than adopt that record silently.
