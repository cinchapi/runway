...
* **Breaking change: preventing stale writes now tests only what a save writes.** A save with `preventStaleWrites` fails when a value it writes changed in the database after the `Record` that holds it last loaded or saved it. Previously the save failed when any value of any record in the object graph changed, so a writer could be rejected over a field it never touched. Each record is now judged against its own writes: a record the save writes nothing to never fails, and a record staged for deletion removes every stored value, so any external change to it still fails the save. The guarantee covers writes, not reads. A caller that reads one value to decide another, or that wants the guarantee enforced structurally, uses a `Transaction` instead, whose reads join its conflict footprint. ([GH-179](https://github.com/cinchapi/runway/issues/179))
    * The test compares the stored value against the one the instance holds, so a value that another writer changed and changed back is not a conflict: the save overwrites nothing that writer left behind.
...
* **A transaction owns its records exclusively.** Every `Record` loaded or created through the view is bound to the transaction, along with the records linked from it: `save()` stages within the transaction and the writes become durable only when `commit()` succeeds.
    ...
    * The single-key atomic operations (`exchange`, `getAndUpdate` and `updateAndGet`) are refused on a bound record because the transaction's commit is the unit of atomicity; after the transaction ends, the operations resume against the enclosing `Runway`.
