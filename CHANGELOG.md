...
##### References
Sometimes a stored reference points at a record that holds no data, because the record was deleted or because a writer outside of Runway removed it. Previously, Runway did one of three things in that situation, and which one a caller got depended on the shape of the field and on the server. A collection dropped the element and quietly deleted the stored reference. A field holding a single record did the same on some servers. On other servers that same field failed the load of the record that held it, so the whole record became unreadable instead of just missing a value. None of the three was something a user could ask for. This release names all three, picks one as the default, and lets a field choose another.

* **A reference that leads to no record resolves to nothing.** A field that holds a single record is left unset, a collection omits the element, and the record that holds the reference loads normally. The stored reference stays where it is, because a load no longer deletes it. Every field behaves this way on every server.
* **Declare `@ReferenceNotFound` on a field to choose a different policy.** `ReferenceNotFoundPolicy.REPAIR` also deletes the stored reference, so the record stops carrying it. `ReferenceNotFoundPolicy.ERROR` fails the load of the record that holds the reference and throws a `ReferenceNotFoundException` naming that record, the field, and the record the reference points at. Use `ERROR` for a reference the record cannot be correct without.
* **`Runway.builder().referenceNotFoundPolicy` sets the policy for every field that declares none.** The default is `ReferenceNotFoundPolicy.SKIP`. Declare `REPAIR` on a field that relied on a load to clean up after a deletion, or set it as the database policy to keep that cleanup everywhere.
