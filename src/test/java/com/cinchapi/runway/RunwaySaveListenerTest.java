...
public class RunwaySaveListenerTest extends RunwayBaseClientServerTest {
    ...
    public void testPreSaveHookCalledBeforeSave() throws Exception {
        ...
        Assert.assertTrue("preSave hook should have been called", record.preSaveCalled);
        Assert.assertEquals("Field should be modified by preSave", "Original Name (Modified)", record.name);
        
        // Load the record to verify the changes were persisted
        PreSaveHookRecord loaded = runway.load(PreSaveHookRecord.class, record.id());
        Assert.assertEquals("Modified value should be persisted", "Original Name (Modified)", loaded.name);
    }
    ...
    public void testPreSaveHookCalledBeforeSaveListener() throws Exception {
        ...
        Assert.assertTrue("preSave should be called before save listener", preSaveCalledBeforeListener.get());
    }
    ...
    public void testPreSaveHookCalledInTransaction() throws Exception {
        ...
        // Verify the save was successful and preSave was called for both records
        Assert.assertTrue("Records should save successfully", saved);
        Assert.assertTrue("preSave hook should have been called for record1", record1.preSaveCalled);
        Assert.assertTrue("preSave hook should have been called for record2", record2.preSaveCalled);
        Assert.assertEquals("Field should be modified by preSave", "Record 1 (Modified)", record1.name);
        Assert.assertEquals("Field should be modified by preSave", "Record 2 (Modified)", record2.name);
    }
    
    @Test
    public void testPreSaveHookExceptionAbortsSave() throws Exception {
        // Create a record with a preSave implementation that throws an exception
        ...
        Assert.assertFalse("Save should fail when preSave throws an exception", saved);
    ...
    @Test
    public void testPreSaveHookExceptionInTransactionAbortsAllSaves() throws Exception {
        ...
        Assert.assertFalse("Transaction should fail when preSave throws an exception", saved);
    ...
    @Test
    public void testSaveListenerNotCalledWhenPreSaveThrowsException() throws Exception {
        ...
        Assert.assertFalse("Save listener should not be called when save fails", listenerCalled.get());
    }
    ...
    public void testPreSaveHookCalledWithBulkSave() throws Exception {
        ...
        for (int i = 0; i < recordCount; i++) {
            Assert.assertTrue("preSave hook should have been called for record " + i, 
                    records[i].preSaveCalled);
            Assert.assertEquals("Field should be modified by preSave", 
                    "Bulk Record " + i + " (Modified)", records[i].name);
            
            // Load the record to verify the changes were persisted
            PreSaveHookRecord loaded = runway.load(PreSaveHookRecord.class, records[i].id());
    ...
    public void testPreSaveHookExceptionAbortsBulkSave() throws Exception {
        // Create multiple records, with one that will throw an exception in preSave
        PreSaveHookRecord record1 = new PreSaveHookRecord();
        PreSaveHookRecord record2 = new PreSaveHookRecord();
        ...
        // Verify the save failed
        Assert.assertFalse("Bulk save should fail when a record's preSave throws an exception", saved);
        
        // Verify no records were persisted
        for (Record record : new Record[] { record1, record2, exceptionRecord, record3 }) {
            try {
                runway.load(record.getClass(), record.id());
                Assert.fail("Record with ID " + record.id() + " should not exist in the database");
            }
            catch (IllegalStateException e) {
                // Expected exception
            }
    ...
    public void testSaveListenerCalledForBulkSave() throws Exception {
        ...
        for (PreSaveHookRecord record : records) {
            Assert.assertTrue("Save listener should have processed record " + record.id(), 
                    savedRecords.contains(record));
            Assert.assertTrue("preSave hook should have been called before save listener", 
                    record.preSaveCalled);
            Assert.assertEquals("Field should be modified by preSave", 
                    "Listener Bulk Record " + records.length + " (Modified)", 
                    record.name);
        }
    }
    ...
    public void testPreSaveHookModifiesFieldsInBulkSave() throws Exception {
        ...
        Assert.assertEquals("Count field should be incremented by preSave", 11, record1.count);
        Assert.assertNotNull("Timestamp field should be set by preSave", record1.timestamp);
        
        Assert.assertTrue("preSave hook should have been called for record2", record2.preSaveCalled);
        Assert.assertEquals("Name field should be modified by preSave", 
                "Complex Record 2 (Modified)", record2.name);
        Assert.assertEquals("Count field should be incremented by preSave", 21, record2.count);
        Assert.assertNotNull("Timestamp field should be set by preSave", record2.timestamp);
        
        // Load the records to verify the changes were persisted
        ComplexPreSaveRecord loaded1 = runway.load(ComplexPreSaveRecord.class, record1.id());
        Assert.assertEquals("Modified name should be persisted", "Complex Record 1 (Modified)", loaded1.name);
        Assert.assertEquals("Modified count should be persisted", 11, loaded1.count);
        Assert.assertNotNull("Timestamp should be persisted", loaded1.timestamp);
        
        ComplexPreSaveRecord loaded2 = runway.load(ComplexPreSaveRecord.class, record2.id());
        Assert.assertEquals("Modified name should be persisted", "Complex Record 2 (Modified)", loaded2.name);
        Assert.assertEquals("Modified count should be persisted", 21, loaded2.count);
        Assert.assertNotNull("Timestamp should be persisted", loaded2.timestamp);
    }
